...
if ( ! function_exists( 'pharmacare_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function pharmacare_core_get_header_logo_image( $parameters = array() ) {
		$header_skin     = pharmacare_core_get_post_value_through_levels( 'qodef_header_skin' );
		$logo_height     = pharmacare_core_get_post_value_through_levels( 'qodef_logo_height' );
		$customizer_logo = pharmacare_core_get_customizer_logo();
		...
		foreach ( $available_logos as $logo ) {
			...
			$logo_image_id = pharmacare_core_get_post_value_through_levels( 'qodef_logo_' . $logo );

			if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
				$logo_image_id = pharmacare_core_get_post_value_through_levels( 'qodef_logo_main' );
			}
			...
			if ( ! empty( $logo_image_id ) ) {
				$logo_image_attr = array(
					...
					'itemprop' => 'image',
					'alt'      => sprintf( esc_attr__( 'logo %s', 'pharmacare-core' ), $logo ),
				);
		...
		if ( ! empty( $customizer_logo ) ) {
			...
			echo apply_filters( 'pharmacare_core_filter_get_header_logo_image', pharmacare_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
		}
	}
}

if ( ! function_exists( 'pharmacare_core_get_header_widget_area' ) ) {
	/**
	 ...
	 */
	function pharmacare_core_get_header_widget_area( $header_layout = '', $widget_area = 'one' ) {
		...
		if ( $is_enabled ) {
			$parameters = apply_filters(
				'pharmacare_core_filter_header_widget_area',
				array(
					'page_id'             => $page_id,
			...
			pharmacare_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}
