...
if ( ! function_exists( 'pharmacare_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pharmacare_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'PharmaCareCore_Icon_Shortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'pharmacare_core_filter_register_shortcodes', 'pharmacare_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'PharmaCareCore_Shortcode' ) ) {
	class PharmaCareCore_Icon_Shortcode extends PharmaCareCore_Shortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( PHARMACARE_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'pharmacare_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'pharmacare-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'pharmacare-core' ) );
			...
			$this->set_option( array(
				'field_type'	=> 'iconpack',
				'name'			=> 'main_icon',
				'title'			=> esc_html__( 'Icon', 'pharmacare-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'size',
				...
				'options'    => array(
					...
					'tiny'    => esc_html__( 'Tiny', 'pharmacare-core' ),
					'small'   => esc_html__( 'Small', 'pharmacare-core' ),
					'medium'  => esc_html__( 'Medium', 'pharmacare-core' ),
					'large'   => esc_html__( 'Large', 'pharmacare-core' ),
					'huge'    => esc_html__( 'Huge', 'pharmacare-core' ),
				)
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'custom_size',
				'title'      => esc_html__( 'Custom Size', 'pharmacare-core' ),
			) );
            $this->set_option( array(
                'field_type' => 'select',
                'name'       => 'icon_weight',
                'title'      => esc_html__( 'Font Weight', 'pharmacare-core' ),
                'options'       => pharmacare_core_get_select_type_options_pool( 'font_weight', true, array(100,200,500,700,800,900) ),
            ) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'icon_layout',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_radius',
				'title'      => esc_html__( 'Border Radius', 'pharmacare-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => 'square',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'shape_size',
				'title'      => esc_html__( 'Shape Size', 'pharmacare-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_width',
				'title'      => esc_html__( 'Border Width', 'pharmacare-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => array( 'circle', 'square' ),
            ...
            $this->set_option( array(
                ...
                'title'      => esc_html__( 'Vertical Offset', 'pharmacare-core' ),
            ) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'link',
				'title'      => esc_html__( 'Icon Link', 'pharmacare-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'target',
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'pharmacare_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return pharmacare_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
		...
		private function generate_icon_styles( $atts ) {
            ...
            if ( ! empty( $atts['icon_weight'] ) ) {
                $styles[] = 'font-weight: ' . $atts['icon_weight'];
            }
