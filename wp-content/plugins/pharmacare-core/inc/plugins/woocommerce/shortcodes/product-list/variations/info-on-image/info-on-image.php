...
if ( ! function_exists( 'pharmacare_core_add_product_list_variation_info_on_image' ) ) {
	/**
	 ...
	 * @return array
	 */
	function pharmacare_core_add_product_list_variation_info_on_image( $variations ) {
		$variations['info-on-image'] = esc_html__( 'Info On Image', 'pharmacare-core' );
		
		return $variations;
	}
	
	add_filter( 'pharmacare_core_filter_product_list_layouts', 'pharmacare_core_add_product_list_variation_info_on_image' );
}

if ( ! function_exists( 'pharmacare_core_register_shop_list_info_on_image_actions' ) ) {
	/**
	 * Function that override product item layout for current variation type
	 */
	function pharmacare_core_register_shop_list_info_on_image_actions() {
		...
		remove_action( 'woocommerce_shop_loop_item_title', 'pharmacare_woo_shop_loop_item_title', 10 ); // permission 10 is default
		add_action( 'woocommerce_before_shop_loop_item_title', 'pharmacare_woo_shop_loop_item_title', 12 ); // permission 17 is set because pharmacare_add_product_list_item_additional_image_holder hook is added on 15
		...
		add_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_price', 14 ); // permission 19 is set because pharmacare_woo_shop_loop_item_title hook is added on 17
		...
		add_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_add_to_cart', 20 ); // permission 20 is set because pharmacare_add_product_list_item_additional_image_holder hook is added on 15
	}
	
	add_action( 'pharmacare_core_action_shop_list_item_layout_info-on-image', 'pharmacare_core_register_shop_list_info_on_image_actions' );
}