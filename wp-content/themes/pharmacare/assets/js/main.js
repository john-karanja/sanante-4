(function ( $ ) {
	...
	$( document ).on(
		'pharmacare_trigger_get_new_posts',
		function () {
			qodefSwiper.init();
			qodefMagnificPopup.init();
		}
	...
	/**
	 * Custom Hover Trigger function
	 *
	 * @param {string} trigger - element to be hovered
	 * @param {string} parent - holder element for class change
	 */
	var qodefHoverTrigger = {
		init: function (trigger, parent) {
			if ($(trigger).length) {
				$(trigger)
				.on('mouseenter', function() {
					$(this).closest(parent).addClass('qodef-hovered');
				})
				.on('mouseleave', function() {
					$(this).closest(parent).removeClass('qodef-hovered');
				});
			}
		}
	};

	qodef.qodefHoverTrigger = qodefHoverTrigger;

    var qodefBgTextAnimation = {
        init: function () {
            var $bgTexts = $('.qodef-custom-font-landing .qodef-custom-font');
            ...
            if ($bgTexts.length) {
                $bgTexts.each(function () {
                    var $thisBgText = $(this),
                        ...
                        thisBgTextArrayLength = thisBgTextArray.length - 1;

                    $thisBgText.empty();
                    // Special case for masked text
                    if ($thisBgText.hasClass('qodef-e-fixed-text-one')) {
                        $thisBgText.append('<span class="qodef-e-fixed-text-one-mask"></span>');
                    }
                    thisBgTextArray.forEach(function (item, index) {
                        if (item === " ") {
                        ...
                        if (index === thisBgTextArrayLength) {
                            setTimeout(function () {
                                $thisBgText.addClass('qodef-bg-text--animated');
                                qodefBgTextAnimation.initAppear($thisBgText);
                            }, 500);
                        }
                    });
                });
        ...
        initAppear: function ($thisBgText) {
            if ( $thisBgText.hasClass('qodef-wait-for-spinner--enabled') ) {
				var $items = $thisBgText.find('.qodef-bg-text-char:not(.qodef--empty-char)');
				...
				$thisBgText.appear(function () {
					$thisBgText.find('.qodef-bg-text-char:not(.qodef--empty-char)').each(function (i) {
						var $thisItem = $(this);
    ...
    qodef.qodefBgTextAnimation = qodefBgTextAnimation;

	var qodefUncoveringSection = {
		init: function () {
			this.holder = $('#qodef-custom-section--uncovering');

			if ( this.holder.length && !qodefCore.html.hasClass('touchevents') && qodef.windowWidth > 1024 ) {
				qodefUncoveringSection.addClass( this.holder );
				qodefUncoveringSection.setHeight( this.holder );
				qodefUncoveringSection.setAppear( this.holder );

				$(window).resize(function () {
					qodefUncoveringSection.setHeight( qodefUncoveringSection.holder );
				});
			}
		},
		...
		addClass: function () {
			qodefCore.body.addClass('qodef-page-has-custom-section--uncovering');
		},
	...
	qodef.qodefUncoveringSection = qodefUncoveringSection;

	var qodefAppear = {
		init: function () {
			this.holder = $('.qodef-appear-animation--enabled:not(.qodef-masonry-elements)');
