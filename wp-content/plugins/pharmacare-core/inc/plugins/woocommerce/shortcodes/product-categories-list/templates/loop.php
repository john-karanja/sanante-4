...
if ( ! empty( $taxonomy_items ) ) {
	foreach ( $taxonomy_items as $taxonomy_item ) {
		...
		pharmacare_core_list_sc_template_part( 'plugins/woocommerce/shortcodes/product-categories-list', 'layouts/' . $layout, '', $params );
	}
} else {
	// Include global posts not found
	pharmacare_core_theme_template_part( 'content', 'templates/parts/posts-not-found' );
}