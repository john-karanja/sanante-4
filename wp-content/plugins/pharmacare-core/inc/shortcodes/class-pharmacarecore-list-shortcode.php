...
abstract class PharmaCareCore_List_Shortcode extends QodeFrameworkShortcode {
	private $post_type;
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => pharmacare_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior, $include_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Image Proportions', 'pharmacare-core' ),
					'options'       => array(
						''      => esc_html__( 'Original', 'pharmacare-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'default_value' => 'full',
					'title'         => esc_html__( 'Image Proportions', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'list_image_dimension', false ),
					'group'         => $group,
					'dependency'    => array(
						'show' => array(
							'behavior' => array(
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Custom Image Width', 'pharmacare-core' ),
					'description' => esc_html__( 'Enter image width in px', 'pharmacare-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Custom Image Height', 'pharmacare-core' ),
					'description' => esc_html__( 'Enter image height in px', 'pharmacare-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
							'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
					'default_value' => '3',
					'group'         => $group,
					'dependency'    => array(
						'hide' => array(
							'behavior' => array(
								'values'        => array('justified-gallery', 'carousel-vertical'),
								'default_value' => 'columns',
							),
						),
					),
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Columns Responsive', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_responsive' ),
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'columns_1440',
					'title'         => esc_html__( 'Number of Columns 1367px - 1440px', 'pharmacare-core' ),
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'columns_1366',
					'title'         => esc_html__( 'Number of Columns 1025px - 1366px', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
					'dependency'    => array(
						'show' => array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'columns_1024',
					'title'         => esc_html__( 'Number of Columns 769px - 1024px', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
					'dependency'    => array(
						'show' => array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'columns_768',
					'title'         => esc_html__( 'Number of Columns 681px - 768px', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
					'dependency'    => array(
						'show' => array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'columns_680',
					'title'         => esc_html__( 'Number of Columns 481px - 680px', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
					'dependency'    => array(
						'show' => array(
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'columns_480',
					'title'         => esc_html__( 'Number of Columns 0 - 480px', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'columns_number' ),
					'default_value' => '3',
					'dependency'    => array(
						'show' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Row Height', 'pharmacare-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Max Row Height', 'pharmacare-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Last Row Treshold', 'pharmacare-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Enable Slider Navigation', 'pharmacare-core' ),
				'options'    => pharmacare_core_get_select_type_options_pool( 'yes_no' ),
				'dependency' => $dependency,
				'group'      => $group,
			)
		...
		$this->set_option(
			array(
				...
				'title'      => esc_html__( 'Enable Slider Pagination', 'pharmacare-core' ),
				'options'    => pharmacare_core_get_select_type_options_pool( 'yes_no' ),
				'dependency' => $dependency,
				'group'      => $group,
			)
	...
	public function get_list_classes( $atts ) {
		...
		$holder_classes[] = ! empty( $atts['behavior'] ) && 'carousel-vertical' === $atts['behavior'] ? 'qodef-swiper-carousel-vertical' : '';
        ...
        if ( isset( $atts['filter_style'] ) && '' !== $atts['filter_style'] ) {
            $holder_classes[] = 'qodef-filter-style--' . $atts['filter_style'];
        }
	...
	public function get_list_item_classes( $atts ) {
		...
		$item_classes[] = ! empty( $atts['behavior'] ) && 'carousel-vertical' === $atts['behavior'] ? 'swiper-slide' : 'qodef-grid-item';
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'masonry' === $atts['behavior'] && ! empty( $atts['masonry_images_proportion'] ) && 'fixed' === $atts['masonry_images_proportion'] ) {
			...
			$image_dimension              = pharmacare_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'pharmacare-core' );
		...
		$this->set_option(
			array(
				'field_type'    => 'text',
				'name'          => 'posts_per_page',
				'title'         => esc_html__( 'Posts per Page', 'pharmacare-core' ),
				'default_value' => '-1',
				'group'         => $group,
			)
		);
		$this->set_option(
			array(
				'field_type'    => 'select',
				'name'          => 'orderby',
				'title'         => esc_html__( 'Order By', 'pharmacare-core' ),
				'options'       => pharmacare_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
				'default_value' => 'date',
				'group'         => $group,
			)
		);
		$this->set_option(
			array(
				'field_type'    => 'select',
				'name'          => 'order',
				'title'         => esc_html__( 'Order', 'pharmacare-core' ),
				'options'       => pharmacare_core_get_select_type_options_pool( 'order' ),
				'default_value' => 'DESC',
				'group'         => $group,
			)
		);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option, true ) ) ) {
			...
			do_action( 'pharmacare_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters(
				'pharmacare_core_filter_map_additional_query_params',
				array(
					''       => esc_html__( 'No', 'pharmacare-core' ),
					'id'     => esc_html__( 'Post IDs', 'pharmacare-core' ),
					'tax'    => esc_html__( 'Taxonomy Slug', 'pharmacare-core' ),
					'author' => esc_html__( 'Author Name', 'pharmacare-core' ),
				)
			);
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'post_ids',
					'title'       => esc_html__( 'Posts IDs', 'pharmacare-core' ),
					'description' => esc_html__( 'Separate post IDs with commas', 'pharmacare-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option(
					array(
						'field_type' => 'select',
						'name'       => 'tax',
						'title'      => esc_html__( 'Taxonomy', 'pharmacare-core' ),
				...
				$this->set_option(
					array(
						'field_type' => 'text',
						'name'       => 'tax_slug',
						'title'      => esc_html__( 'Taxonomy Slug', 'pharmacare-core' ),
						'group'      => $group,
						'dependency' => array(
							'show' => array(
								'additional_params' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'author_slug',
					'title'      => esc_html__( 'Author Slug', 'pharmacare-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			do_action( 'pharmacare_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'pharmacare_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'pharmacare-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_filter',
					'title'         => esc_html__( 'Enable Filter', 'pharmacare-core' ),
					'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'pharmacare-core' ),
            ...
            $this->set_option( array(
                'field_type'  => 'select',
                'name'        => 'filter_style',
                'title'       => esc_html__( 'Filter Style', 'pharmacare-core' ),
                'options'     => array(
                    ''        => esc_html__( 'Default', 'pharmacare-core' ),
                    'squares' => esc_html__( 'Squares', 'pharmacare-core' )
                ),
                'dependency'    => array(
                    'show' => array(
                        'behavior' => array(
			...
			do_action( 'pharmacare_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Pagination', 'pharmacare-core' ),
					'options'       => pharmacare_core_get_select_type_options_pool( 'pagination_type' ),
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Load More Top Margin (px or %)', 'pharmacare-core' ),
					'default_value' => '',
					'group'         => $group_name,
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'pharmacare_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'pharmacare_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );

		$this->set_scripts( $list_scripts );
	}

	public function load_assets() {
		do_action( 'pharmacare_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
