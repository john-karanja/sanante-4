(function ($) {
	...
	var qodefLoginModal = {
		...
		triggerShowModal: function ($holder) {
			...
			// close button
			$holder.find('.qodef-membership-login-modal-close').on('click', function ( e ) {
				e.preventDefault();
				qodefLoginModal.hideModal($holder);
			});
			...
			$(document.body).on('pharmacare_membership_trigger_login_modal', function () {
		...
		triggerFormSocialSubmit: function ($holder) {
			...
			if ($form.length) {
				...
				$socialButton.find('.qodef-m-social-login-btn').on('click', function (e) {
					...
					$(document).trigger('pharmacare_membership_social_login_is_triggered', [qodefLoginModal, $form, $(this).data('social')]);
				});
			}
		},
		triggerRequest: function ($holder, socialNetwork, socialResponse) {
			...
			var ajaxData = {
				...
				nonce: $holder.find('#pharmacare-membership-ajax-' + $requestType + '-nonce').val()
			};
		...
		triggerForceLogin: function ($holder, ajaxData, redirect) {
			...
			ajaxData.nonce = $holder.parent().find('#pharmacare-membership-ajax-login-nonce').val();
	...
	var qodefLoginOpener = {
		...
		triggerClick: function ($holder) {
			...
			$opener.on('click', function (e) {
				...
				$(document.body).trigger('pharmacare_membership_trigger_login_modal');
			});
		}
	};
...
(function ($) {
	...
	$(document).on('pharmacare_membership_social_login_is_triggered', function (e, $modal, $form, social) {
	...
	var qodefFacebookLogin = {
		init: function () {
			...
			if (qodefFacebookLogin.fbIsAppIdSet()) {
				qodefFacebookLogin.fbAsyncInit(pharmacareMembershipGlobal.facebookAppId);
			}
		},
		fbIsAppIdSet: function () {
			return typeof pharmacareMembershipGlobal.facebookAppId !== 'undefined' && pharmacareMembershipGlobal.facebookAppId !== '';
		},
	...
	var qodefGoogleLogin = {
		init: function () {
			...
			if (qodefGoogleLogin.isAppIdSet()) {
				qodefGoogleLogin.asyncInit(pharmacareMembershipGlobal.googleAppId);
			}
		},
		isAppIdSet: function () {
			return typeof pharmacareMembershipGlobal.googleAppId !== 'undefined' && pharmacareMembershipGlobal.googleAppId !== '';
		},
