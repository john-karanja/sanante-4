...
abstract class PharmaCareCore_Header {
	...
	public function __construct() {
		...
		add_filter( 'pharmacare_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		...
		add_filter( 'pharmacare_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'pharmacare_filter_header_inner_class', array( $this, 'set_header_inner_classes' ), 10, 2 );
		add_filter( 'pharmacare_filter_mobile_header_inner_class', array( $this, 'set_header_inner_classes' ), 10, 2 );
		add_filter( 'pharmacare_core_filter_nav_menu_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'pharmacare_core_filter_nav_menu_narrow_header_selector', array( $this, 'set_nav_menu_narrow_header_selector' ) );
	}
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'pharmacare_core_filter_header_template', $parameters );

		return pharmacare_core_get_template_part( 'header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = pharmacare_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
	...
	function set_header_height() {
		$header_height = pharmacare_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'pharmacare_core_filter_set_header_height', $header_height );
	}

	function set_header_inner_classes( $class, $layout ) {
		// Check is content in grid
		$class[] = 'yes' === pharmacare_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_in_grid' ) ? 'qodef-content-grid' : '';

		// Check header skin
		$header_skin = pharmacare_core_get_post_value_through_levels( 'qodef_header_skin' );
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = pharmacare_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$background_color = pharmacare_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
		...
		$side_padding = pharmacare_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_padding' );
