...
if ( ! class_exists( 'PharmaCareMembershipTwitterApi' ) ) {
	/**
	 * Class PharmaCareMembershipTwitterApi
	 */
	class PharmaCareMembershipTwitterApi {
		/**
		 * @var $instance PharmaCareMembershipTwitterApi class object
		 */
		...
		/**
		 * @return PharmaCareMembershipTwitterApi
		 */
		 ...
		 *
		 * @see PharmaCareMembershipTwitterApi::generateSignatureBase()
		 */
		...
		public function obtainRequestToken() {
			...
			if ( is_wp_error( $response ) ) {
				...
				$responseObj->message = esc_html__( 'Internal WP error', 'pharmacare-membership' );
			} else {
				...
				if ( ! empty( $responseBody ) ) {
					...
					if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) && ! empty( $responseParsed['oauth_token'] ) && ! empty( $responseParsed['oauth_token_secret'] ) ) {
						...
						if ( empty( $responseObj->redirectUrl ) ) {
							...
							$responseObj->message                  = esc_html__( 'Redirect URL could not be generated', 'pharmacare-membership' );
						} else {
							$responseObj->oauth_callback_confirmed = true;
						}
					} else {
						$responseObj->oauth_callback_confirmed = false;
						$responseObj->message                  = esc_html__( 'Couldn\'t connect with Twitter API', 'pharmacare-membership' );
					}
				}
			}
		...
		public function buildCurrentPageURI() {
			...
			return pharmacare_membership_get_membership_redirect_url();
		}
	}
}
